...
import { sendNONNative } from '@/function';
...
const page = ({ searchParams }) => {
    ...
    const send = async () => {
        if(amount == null || destinationAddress == 'Enter Recipient Address' || destinationAddress == null || destinationAddress == undefined){
            alert('Enter a valid Recipient Address and amount')
        }else{
            setIsLoading(true);
            let key = auth.mainAccount.privateKey || auth.mainAccount.secretKey
            let transact = await sendNONNative(key , destinationAddress , amount , data.address);
            // alert(JSON.stringify(transact))
            if(transact.success){
                // alert(JSON.stringify(transact.transactionid))
                        ...
                        <>
                            <div className="mb-4">
                                ...
                                <div className="flex justify-between mt-2 text-gray-600">
                                    ...
                                    <span className='text-sm text-black p-1 mt-1'><span className='font-bold text-gray-500'>Available Balance:</span> {walletBalance} SOL</span>
                                </div>
                            </div>
                            ...
                            <div onClick={send} className='mb-4'>
